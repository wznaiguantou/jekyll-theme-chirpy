...
- SpringApplication Instance.run(args) -- 依赖第一步的实例真实进行启动
...
源文件我就不展示了，但是我们需要注意一点的地方是，这3个jar中定义的部分内如是重复的，所以Spring Boot 在启动加载的时候会进行去重处理。我们这里给一个json样式最后处理得到的结果
